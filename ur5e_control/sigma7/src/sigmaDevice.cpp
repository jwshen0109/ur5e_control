        ...
        : new_wrench_msg(false)
{
    ...
    pub_pose = n.advertise<geometry_msgs::PoseStamped>("pose",1, 0);
...
int SigmaDevice::CalibrateDevice() {
    ...
    else if(drdStart((char)id) < 0){
        ROS_ERROR("Error: regulation thread failed to start %i, and says: (%s)",id,dhdErrorGetLastStr());
        dhdSleep(2.0);
    }
...
int SigmaDevice::ReadMeasurementsFromDevice() {
    ...
    tf::Stamped<tf::Transform> sigma_transform;
    pose_msg.pose.position.x -= 0.5;
    pose_msg.pose.position.z += 0.5;
    tf::poseStampedMsgToTF(pose_msg, sigma_transform);
    tf_pub.sendTransform(tf::StampedTransform(sigma_transform,sigma_transform.stamp_,"world","hand"));
...
void SigmaDevice::setForceAndTorque(){
